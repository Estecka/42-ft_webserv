...
/*   Updated: 2021/10/23 16:16:31 by abaur            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool	UriExtensionMatch(const std::string& uri, const std::string& loc){
	...
	if (loc.size() < 3 || loc[0]!='*' || loc[1]!='.') {
		ft::clog << "[ERR] Invalid extension expression: " << loc << std::endl;
		return false;
	}
...
namespace ft
{
	...
	bool	UriConfig::UriMatchHandle(const std::string& uri, const LocationHandle& handle){
		switch (handle.prefix)
		{
			...
			default:
				ft::clog << "[ERR] Invalid location prefix: " << handle.prefix << std::endl;
				return false;
		}
	}
	...
	void UriConfig::AddProperties(const PropertyList& properties){
		for (PropertyList::const_iterator it=properties.begin(); it!=properties.end(); it++) {
			if (it->first == "root")
				...
				ft::clog << "[WARN] Unknown instruction name: " << it->first << std::endl;
		}
	}
	...
	void	UriConfig::ParseAutoindex(const std::string& raw) {
		if (raw == "on")
			...
			ft::clog << "[WARN] Invalid autoindex value: " << raw << std::endl;
	}
	...
	void	UriConfig::ParseReturn(const std::string& raw){
		...
		if (code.empty()) {
			ft::clog << "[WARN] Empty return code" << std::endl;
			return;
		}

		for (size_t i=0; i<code.length(); i++)
		if (!std::isdigit(code[i])) {
			ft::clog << "[WARN] Invalid return code: " << code << std::endl;
			return;
		}
