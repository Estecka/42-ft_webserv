...
/*   Updated: 2021/10/23 16:10:06 by abaur            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
namespace ft
{
	ReqBodyExtractor::ReqBodyExtractor(RequestHandler& parent) :
		...
		_outEof(false)
	{
		ft::clog << "[DEBUG] BodyExtractor Created" << std::endl;
		_body = std::tmpfile();
		if (_body == NULL)
	...
	ReqBodyExtractor::~ReqBodyExtractor(){
		if (this->_body)
			std::fclose(_body);
		ft::clog << "[DEBUG] BodyExtractor destroyed." << std::endl;
	}
	...
	bool	ReqBodyExtractor::read() {
		// ft::clog << "[DEBUG] About to read body...\n";
		_httpin.clear();
		while (true) 
		{
			...
			if (_buffend > 0) {
				...
				if (_inFail) {
					// ft::clog << "[DEBUG] " << _bodylen << " vs " << _parent.GetReqHead()->GetContentLength() << std::endl;
	...
	bool	ReqBodyExtractor::write() {
		// ft::clog << "[DEBUG] About to write to temp file...\n";
		std::clearerr(_body);
		while (true) {
